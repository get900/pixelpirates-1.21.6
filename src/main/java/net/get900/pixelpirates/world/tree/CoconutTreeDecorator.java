...
public class CoconutTreeDecorator extends TreeDecorator {
    ...
    public CoconutTreeDecorator(float chance) {
        this.chance = chance;
    }

    @Override
    protected TreeDecoratorType<?> getType() {
        return ModTreeDecorator.COCONUT;
    }
    ...
    public void generate(Generator generator) {
        ...
        for (BlockPos logPos : generator.getLogPositions()) {
            if (random.nextFloat() < this.chance) {
                ...
                BlockPos coconutPos = logPos.offset(direction);
                if (generator.isAir(coconutPos)) {
                    generator.replace(coconutPos, ModBlocks.COCONUT_BLOCK.getDefaultState());
                }
            }
        }
    }
